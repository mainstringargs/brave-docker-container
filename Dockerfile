FROM ubuntu:14.04
RUN apt-get update && apt-get install -y x11-xserver-utils && apt-get install -y curl && apt-get install -y dbus-x11 && apt-get install --force-yes  -y apt-transport-https && useradd --create-home browser
